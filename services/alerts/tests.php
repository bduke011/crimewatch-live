...
$s=['enabled'=>true,'nearby'=>true,'daily'=>true,'updates'=>true,'agency'=>'polk','center'=>[30.79,-94.90],'radius'=>5,'categories'=>['property'],'hour'=>0,'timezone'=>'America/Chicago'];$s=ca_settings($s);check($s['locationMode']==='fixed','Old settings default to fixed location');
...
$moving=$s;$moving['locationMode']='current';$moving['locationUpdatedAt']=$now;
$near=['agency'=>'other-agency','category'=>'property','lat'=>30.79,'lng'=>-94.9];check(ca_matches($near,$moving),'Current location includes all participating agencies');
check(ca_fresh($moving,$now+60),'Fresh current location');check(!ca_fresh($moving,$now+21601),'Old current location expires');
$moving['locationUpdatedAt']=$now-22000;$d['settings']=json_encode($moving);$d['near_cursor']=0;$d['daily_cursor']=0;$d['update_cursor']=0;$d['daily_day']='';$d['watched']='{"old":0}';$plans=ca_plan($db,$d,$now+5);check(count($plans)===1&&$plans[0]['kind']==='updates','Stale location pauses nearby and summary but keeps saved updates');
echo "PASS: current-location cross-agency matching, expiry, and saved-update independence.\n";
