...
test('saving account preferences continues to reports even after notification permission fails', async()=>{
 const elements={}; const element=id=>elements[id]??=( {hidden:false,textContent:'',classList:{toggle(){}},elements:{name:{value:''},area:{value:''}},append(){},insertBefore(){},after(){},scrollIntoView(){},querySelector(){return save;}} );
 ...
 const context={URLSearchParams,document:{getElementById:element,createElement:()=>({hidden:false,innerHTML:'',after(){}})},window:{CW_NATIVE:true,CWNotificationStatus:async()=>false,CWNotifications:async()=>{throw Error('Permission denied');}},localStorage:{setItem(){}},location:{hash:'',replace:value=>route=value},FormData:class{*[Symbol.iterator](){yield ['name','Reader'];yield ['area','houston'];}},fetch:async(url,options)=>{if(options){saved=JSON.parse(options.body);return {ok:true,json:async()=>({reader:{...reader,...saved}})};}return {ok:true,json:async()=>({reader})};},setTimeout};
