...
try{
 ...
 $hasBid=(bool)$db->query("SELECT COUNT(*) FROM pragma_table_info('bookings') WHERE name='bid'")->fetchColumn();
 $st=$db->prepare('SELECT id,name,age,booked,released,locality,details,report_date'.($hasBid?',bid':'').' FROM bookings'.$clause.' ORDER BY booked DESC,name,id LIMIT 24 OFFSET '.(($page-1)*24));$st->execute($args);$rows=$st->fetchAll();foreach($rows as &$r){$r['arrests']=json_decode($r['details'],true,512,JSON_THROW_ON_ERROR);unset($r['details']);}unset($r);
 $photos=[];$bids=array_values(array_filter(array_column($rows,'bid')));if($bids&&$db->query("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN ('roster','takedowns')")->fetchColumn()==2){$st=$db->prepare('SELECT r.bid FROM roster r LEFT JOIN takedowns t ON t.bid=r.bid WHERE r.photo IS NOT NULL AND t.bid IS NULL AND r.bid IN ('.implode(',',array_fill(0,count($bids),'?')).')');$st->execute($bids);$photos=array_flip($st->fetchAll(PDO::FETCH_COLUMN));}
 foreach($rows as &$r){$r['photo_bid']=isset($r['bid'])&&isset($photos[$r['bid']])?$r['bid']:null;unset($r['bid']);}unset($r);
