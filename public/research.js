...
function courtHTML(d){
 if(!d.records.length)return '<div class="source-empty">No published lookup matches this search in the saved collection. This does not mean there are no court cases; most people have not been researched yet.</div>';
 return d.records.map((r,i)=>`<article class="candidate"><div class="candidate-heading"><h3>${esc(r.name)}</h3><span class="match-label">Identity not verified</span></div><dl class="candidate-facts"><div><dt>Export date</dt><dd>${esc(date(r.observed_date))}</dd></div><div><dt>Booking referenced</dt><dd>${esc(date(r.booked))}</dd></div><div><dt>Reported case matches</dt><dd>${r.cases_total}</dd></div></dl><p class="record-status">${r.booking_comparison==='different_date'?`Booking-date mismatch: the current roster lists ${r.roster_dates.map(date).map(esc).join(', ')}. Do not treat this as a confirmed booking link. `:''}Polk County Tyler portal · Owner-supplied lookup summary. Case counts can include civil matters and do not establish convictions.</p><details><summary>${r.references.length} supplied case reference${r.references.length===1?'':'s'} · View details</summary><ul class="research-charges">${r.references.map(ref=>`<li><strong>Case ${esc(ref)}</strong><p>Detailed charges, case status and outcomes are not included in this saved summary.</p></li>`).join('')||'<li>No individual case references were supplied.</li>'}</ul><p class="source-meta">This is a saved search summary, not a current docket. A zero count reports only the result of this imported lookup.</p></details><div class="candidate-actions"><label><input type="checkbox" data-source="court" data-index="${i}" ${selected.has(r.key)?'checked':''}> I reviewed the identifiers; include this lookup</label></div></article>`).join('');
}
...
async function searchSource(source,page=1){
 ...
 try{
  ...
  results[source]=data;$(source+'Badge').textContent=source==='court'?'Searched · saved snapshots':data.warnings.length?'Searched · coverage warning':'Searched';$(source+'Badge').className='source-badge '+(data.warnings.length?'warning':'success');
