...
function syncLinks(){const name=$('personName').value.trim(),q=encodeURIComponent(name);$('rosterLink').href='roster.html?q='+q;$('archiveLink').href='jail.html?q='+q;$('copyName').disabled=!name;}
...
$('copyName').onclick=async()=>{try{await navigator.clipboard.writeText($('personName').value.trim());$('researchStatus').textContent='Name copied. Paste it into the external court portal.';}catch{$('personName').focus();$('personName').select();$('researchStatus').textContent='Copy the highlighted name and paste it into the court portal.';}};
