...
const sourceName=s=>({archive:'Booking archive',roster:'Current roster',court:'Saved court lookup'}[s]);
const params=new URLSearchParams(location.search),sources=['court','archive','roster'];
...
function courtStatus(title,detail,ready=false){
 $('courtAvailability').textContent=title;$('courtAvailabilityDetail').textContent=detail;
 $('courtJump').href=ready?'#courtPanel':'#researchForm';$('courtJump').textContent=ready?'View court results ↓':'Search court records →';
}
function courtHTML(d){
 if(!d.records.length)return '<div class="source-empty">No saved court lookup matches this name. Most people have not been researched yet; this does not mean they have no court history.</div>';
 return d.records.map((r,i)=>`<article class="candidate court-candidate"><div class="candidate-heading"><div><span class="court-saved-label">SAVED COURT SUMMARY</span><h3>${esc(r.name)}</h3></div><span class="match-label">Identity not verified</span></div><div class="court-counts"><div><strong>${r.cases_total}</strong><span>reported case matches</span></div><div><strong>${r.references.length}</strong><span>case numbers supplied</span></div></div><dl class="candidate-facts"><div><dt>Export date</dt><dd>${esc(date(r.observed_date))}</dd></div><div><dt>Booking referenced</dt><dd>${esc(date(r.booked))}</dd></div></dl><p class="record-status">${r.booking_comparison==='different_date'?`Booking-date mismatch: the current roster lists ${r.roster_dates.map(date).map(esc).join(', ')}. Do not treat this as a confirmed booking link. `:''}Polk County Tyler portal · Owner-supplied lookup summary. Counts can include civil matters and do not establish convictions.</p><h4 class="court-reference-title">${r.references.length?'Saved case numbers':'Case numbers not supplied'}</h4><ul class="court-references">${r.references.map(ref=>`<li>Case <strong>${esc(ref)}</strong></li>`).join('')}</ul><p class="source-meta">${r.cases_total>0&&!r.references.length?'The upload reports case matches but does not list their case numbers. ':''}Full court documents, detailed charges, case status and outcomes are not included in this summary.${r.cases_total===0?' A zero count describes this saved lookup only.':''}</p><div class="candidate-actions"><label><input type="checkbox" data-source="court" data-index="${i}" ${selected.has(r.key)?'checked':''}> I reviewed the identifiers; include this lookup</label></div></article>`).join('');
}
function render(source){
 const d=results[source];if(!d)return;
 $(source+'Results').innerHTML=source==='court'?courtHTML(d):d.records.length?d.records.map((r,i)=>`<article class="candidate"><div class="candidate-heading"><h3>${esc(r.name)}</h3><span class="match-label">${r.key===startingKey?'Starting booking':'Possible match'}</span></div><dl class="candidate-facts"><div><dt>Booked</dt><dd>${esc(date(r.booked))}</dd></div><div><dt>Age in record</dt><dd>${esc(r.age??'Not reported')}</dd></div><div><dt>Locality</dt><dd>${esc(r.locality||'Not reported')}</dd></div></dl><p class="record-status">${source==='archive'?(r.released?'Release date recorded: '+esc(date(r.released)):'Release not reported in this snapshot'):'Listed on the roster as of '+esc(updated(d.checkedAt))}</p><details><summary>${r.charges.length} listed charge${r.charges.length===1?'':'s'} · View details</summary><ul class="research-charges">${r.charges.map(chargeHTML).join('')||'<li>No charge details are available.</li>'}</ul>${source==='archive'?`<p class="source-meta">Report snapshot: ${esc(date(r.report_date))}</p>`:`<p class="source-meta">Charge details last read: ${esc(updated(r.last_detail))}. Listed bonds and hearings may change.</p>`}</details><div class="candidate-actions"><a href="${esc(referenceLink(r))}">View full booking →</a><label><input type="checkbox" data-source="${source}" data-index="${i}" ${selected.has(r.key)?'checked':''}> I reviewed the identifiers; include this record</label></div></article>`).join(''):'<div class="source-empty">No matching records in this source for this search. Try a shorter name or another spelling. This is not a finding of no history.</div>';
...
async function searchSource(source,page=1){
 ...
 if(source==='court')courtStatus('Checking saved court records…','Searching for '+currentQuery+'.');
 ...
 try{
  const response=await fetch((source==='court'?'court-api.php?':'research-api.php?')+new URLSearchParams({name:currentQuery,source,page:String(page)}),{signal:c.signal});const data=await response.json();if(!response.ok)throw Error(data.error||'Source unavailable');
  if(c.signal.aborted||g!==generation)return;
  results[source]=data;if(source==='court')courtStatus(data.total?'Saved court information found':'No saved court lookup found',data.total?`${data.total} matching saved ${data.total===1?'summary':'summaries'} for “${currentQuery}”. Review the names and available case numbers below.`:`No saved lookup matches “${currentQuery}”. Try another spelling; coverage is limited.`,true);$(source+'Badge').textContent=source==='court'?(data.total?'Saved results available':'No saved match'):data.warnings.length?'Searched · coverage warning':'Searched';$(source+'Badge').className='source-badge '+(source==='court'||!data.warnings.length?'success':'warning');
  $(source+'Meta').textContent=source==='court'?`${data.total} matching saved lookup${data.total===1?'':'s'}. ${data.coverage.lookups} published lookups in this collection. Export dates: ${date(data.coverage.earliest)} – ${date(data.coverage.latest)}. Identity links have not been independently verified.`:`${data.total} possible matching record${data.total===1?'':'s'}. Last collection: ${updated(data.checkedAt)}.`+(data.coverage?` Archive dates: ${date(data.coverage.earliest)} – ${date(data.coverage.latest)}; gaps may exist.`:'')+' '+data.warnings.join(' ');
  render(source);
 }catch(e){if((e.name==='AbortError'&&!timedOut)||g!==generation)return;delete results[source];if(source==='court')courtStatus('Court search temporarily unavailable','Retry the court search below.',true);$(source+'Badge').textContent='Unavailable';$(source+'Badge').className='source-badge warning';$(source+'Meta').textContent='This source was not searched successfully.';$(source+'Results').innerHTML=`<div class="source-empty">${esc(timedOut?'This source took too long to respond. Try again.':e.message)} <button type="button" data-retry="${source}">Retry source</button></div>`;}
