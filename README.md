...
Optional push alerts are controlled in Settings. Nearby reports use a fixed center or the device’s current location, a radius, and selected categories. Current-location mode covers all participating agencies within the radius. Daily summaries use the device time zone and chosen hour. Saved-report alerts watch published content changes. Alerts start off; each type and the master switch can be disabled. No accounts or advertising are used. My location requests When In Use permission; current-location alerts can optionally request Always permission for significant-change background updates. The server retains only the latest rounded location, and pauses location-based alerts after six hours without a fresh update.
